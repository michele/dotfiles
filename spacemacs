        ...
        (save-excursion
          (let*
              ((from (save-restriction
               ...
               (account
                (cond
                 ((string-match "m@finotto.org" from) "finotto")
                 ((string-match "michele@nearit.com" from) "nearit"))))
            (setq message-sendmail-extra-arguments (list '"-a" account))))))
  (setq mu4e-sent-messages-behavior
        (lambda ()
          (if (string= (message-sendmail-envelope-from) "michele@nearit.com")
              'delete
            'sent)))
...
;;  (add-to-list 'mu4e-view-actions '("View in browser" . mu4e-action-view-in-browser) t)
  ...
  (setq mu4e-contexts
	`( ,(make-mu4e-context
	     :name "finotto"
	     :enter-func (lambda () (mu4e-message "Switch to the finotto context"))
	     :match-func (lambda (msg)
			   (when msg
			     (mu4e-message-maildir-matches msg "^/finotto")))
	     :leave-func (lambda () (mu4e-clear-caches))
	     :vars '((user-mail-address     . "m@finotto.org")
               (user-full-name        . "Michele Finotto")
               (mu4e-get-mail-command . "mbsync -V finotto")
		     (mu4e-sent-folder      . "/finotto/Sent")
		     (mu4e-drafts-folder    . "/finotto/Drafts")
		     (mu4e-trash-folder     . "/finotto/Trash")
		     (mu4e-refile-folder    . "/finotto/Archive")))
	   ,(make-mu4e-context
	     :name "nearit"
	     :enter-func (lambda () (mu4e-message "Switch to the nearit.com context"))
	     :match-func (lambda (msg)
			   (when msg
			     (mu4e-message-maildir-matches msg "^/nearit")))
	     :leave-func (lambda () (mu4e-clear-caches))
	     :vars '((user-mail-address     . "michele@nearit.com")
		     (user-full-name        . "Michele Finotto")
         (mu4e-get-mail-command . "mbsync -V nearit")
		     (mu4e-sent-folder      . "/nearit/[Gmail].Sent Mail")
		     (mu4e-drafts-folder    . "/nearit/[Gmail].Drafts")
		     (mu4e-trash-folder     . "/nearit/[Gmail].Trash")
		     (mu4e-refile-folder    . "/nearit/[Gmail].All Mail"))))))
